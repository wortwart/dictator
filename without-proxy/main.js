'use strict';
const debug = true;
const speechRecogKey = 'XXX';
const spellcheckKey = 'XXX';
const textAnalyticsKey = 'XXX';
const emotionKey = 'XXX';
...
const speechSetup = () => {
	...
	const authentication = new SDK.CognitiveSubscriptionKeyAuthentication(speechRecogKey);
...
const onRequestSpellcheck = ev => {
	...
	hr.open('POST', 'https://api.cognitive.microsoft.com/bing/v5.0/spellcheck?mkt=de-DE&setLang=' + languageOptions.value + '&mode=' + (languageOptions.value === 'en-US'? 'proof' : 'spell'));
	hr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	hr.setRequestHeader('Ocp-Apim-Subscription-Key', spellcheckKey);
...
const onRequestAnalysis = ev => {
	...
	const analyzeText = (type, lang) => {
		...
		hr.open('POST', 'https://westus.api.cognitive.microsoft.com/text/analytics/v2.0/' + type);
		hr.setRequestHeader('Content-Type', 'application/json');
		hr.setRequestHeader('Accept-Type', 'application/json');
		hr.setRequestHeader('Ocp-Apim-Subscription-Key', textAnalyticsKey);
...
const onEmotion = ev => {
	...
	hr.open('POST', 'https://westus.api.cognitive.microsoft.com/emotion/v1.0/recognize');
	hr.setRequestHeader('Content-Type', 'application/octet-stream');
	hr.setRequestHeader('Ocp-Apim-Subscription-Key', emotionKey);
